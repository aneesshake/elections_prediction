...
estimate_votes <- cell_counts %>% 
  mutate(predict_prop = proportion * estimate) %>% 
  group_by(total_num,state_name) %>% 
  summarise(predicted_vote = sum(predict_prop)) %>% 
  mutate(state_prediction = total_num*predicted_vote)
...
cell_counts %>% arrange(num_records)
