...
knitr::kable(c, caption = " Table 1: Model Coefficients") %>% kableExtra::kable_styling(latex_options = "scale_down")
...
cell_counts$estimate <- predict(final_model, newdata = cell_counts)

# post-stratified estimates - number of trump votes
estimate_votes <- cell_counts %>% mutate(predict_prop = proportion * estimate) %>% group_by(state_name) %>% summarise(pred_vote = sum(predict_prop),
                                                                                                                      mean_vote = mean(predict_prop))
estimate_votes
