...
cell_counts$estimate <- model %>% predict(newdata = cell_counts)

cell_counts%>% mutate(predict_prop = estimate*proportion) %>% group_by(state_name) %>% summarize(alp_predict = sum(predict_prop))
