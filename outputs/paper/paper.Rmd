...
Figure (@fig) indicates that a man is much more likely to vote for Trump than a woman is. This tren
...
```{r}
post_strat_estimates %>% group_by(conservative_election_status) %>% summarise(num_states = n()) %>% rename(election_status = conservative_election_status) %>%  knitr::kable(caption = "Outcome of election")
```
